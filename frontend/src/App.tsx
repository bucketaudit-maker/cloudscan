...
export default function App() {
  ...
  const loadMonitor = async() => { setView('monitor'); const [wl,al,dash] = await Promise.all([apiFetch('/monitor/watchlists'),apiFetch('/monitor/alerts'),apiFetch('/monitor/dashboard')]); if(wl?.items)setWatchlists(wl.items); if(al)setAlerts(al); if(dash)setMonDash(dash) }
