  ...
  return <div style={{background:'var(--bg-secondary)',border:'1px solid var(--accent)',borderRadius:12,padding:20,marginBottom:24,animation:'glow 3s ease-in-out infinite'}}>
    ...
    {events.length>0 && <div style={{marginTop:12,maxHeight:150,overflow:'auto'}}>
      {events.slice(-8).reverse().map((e:any,i:number)=><div key={i} style={{display:'flex',alignItems:'center',gap:8,padding:'3px 0',fontSize:11,color:'var(--text-secondary)'}}>
        ...
        {e.bucket?.company_name && <span style={{color:'var(--info)',fontSize:10,fontWeight:600}}>{e.bucket.company_name}</span>}
  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--bg-primary)',color:'var(--text-primary)',fontFamily:'var(--font-body)'}}>
      ...
      {view==='buckets' && <div style={{padding:'80px 24px 24px',maxWidth:1200,margin:'0 auto'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'28px 1fr 100px 95px 85px 75px 90px 85px 85px 75px',gap:12,padding:'8px 16px',fontSize:10,color:'var(--text-muted)',fontWeight:600,textTransform:'uppercase' as const,letterSpacing:'1px',borderBottom:'1px solid var(--border-subtle)'}}><span>★</span><span>Bucket</span><span>Company</span><span>Provider</span><span>Region</span><span>Status</span><span>Risk</span><span>Files</span><span>Size</span><span>Scanned</span></div>
        {buckets?.items?.map((b:any,i:number)=><div key={b.id} onClick={()=>loadBd(b.id)} style={{display:'grid',gridTemplateColumns:'28px 1fr 100px 95px 85px 75px 90px 85px 85px 75px',gap:12,padding:'12px 16px',alignItems:'center',cursor:'pointer',background:i%2===0?'var(--bg-secondary)':'transparent',borderRadius:4}}>
          ...
          <span style={{fontSize:12,color:b.company_name?'var(--info)':'var(--text-muted)',fontWeight:b.company_name?600:400}} title={b.company_name||'No company'}>{b.company_name||'—'}</span>
      ...
      {view==='bucket-detail' && (bd ? <div style={{padding:'80px 24px 24px',maxWidth:1200,margin:'0 auto'}}>
        ...
        <div style={{padding:24,marginBottom:24}} className="card-static">
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:16}}>{[['Company',bd.company_name],['URL',bd.url],['Region',bd.region||'Global'],['Files',fnum(bd.file_count)],['Size',fmt(bd.total_size_bytes)],['First Seen',bd.first_seen?.split('T')[0]],['Last Scanned',ago(bd.last_scanned)]].map(([l,v]:any)=><div key={l}><div style={{fontSize:10,color:'var(--text-muted)',textTransform:'uppercase' as const,marginBottom:4}}>{l}</div><div style={{fontSize:13,color:l==='Company'&&v?'var(--info)':'var(--text-secondary)',fontWeight:l==='Company'&&v?600:400,wordBreak:'break-all' as const}}>{v||'—'}</div></div>)}</div></div>
